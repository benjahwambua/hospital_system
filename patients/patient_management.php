...
if (isset($_POST['register_patient'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO encounters (patient_id, type, status, presenting_complaint, created_at) VALUES (?, 'walkin', 'open', ?, NOW())");
    $stmt->bind_param("is", $patient_id, $complaint);
    $stmt->execute();
...
if (isset($_POST['dispense_medicine'])) {
    ...
    if (!$med || $med['quantity'] < $qty) { $msg = "Invalid med or insufficient stock"; }
        ...
        $invoice_no = get_invoice_number($conn, $invoice_id) ?: $invoice_id;
        ...
        // Post journal entries for medicine sale
        $note = 'Medicine sale: ' . $med['drug_name'] . ' - ' . $qty . ' units';
        post_journal_entry($conn, 'Inventory', $total, 0, $note, $invoice_id);
        post_journal_entry($conn, 'Sales Revenue', 0, $total, $note, $invoice_id);
        
        // Post payment journal entry (marked as paid upon invoice creation)
        post_payment_journal($conn, $invoice_id, $total, $payment_mode);
